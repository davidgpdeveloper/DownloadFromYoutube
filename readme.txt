...
if "%ARG_PROCESSED%"=="0" (
    if "%~1"=="" (
        rem Leer URL desde PowerShell para preservar caracteres especiales como &
        for /f "usebackq delims=" %%A in (`powershell -NoProfile -Command "Read-Host '🔗 Introduce la URL del video (deja vacio para salir)'"`) do set "URL=%%A"
        if not defined URL (
            rem Fallback a set /p si PowerShell no esta disponible
...
rem Recortar parametros de listas de reproduccion si se detecta &list= o ?list=
set "URL_TEMP=%URL:&list=|=%"
if not "%URL_TEMP%"=="%URL%" (
    for /f "tokens=1 delims=|" %%L in ("%URL_TEMP%") do set "URL=%%L"
    echo ⏳ Se detecto un parametro de lista y la URL se ha recortado a:
    echo %URL%
) else (
    set "URL_TEMP=%URL:?list=|=%"
    if not "%URL_TEMP%"=="%URL%" (
