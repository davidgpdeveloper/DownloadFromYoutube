...
rem Script portátil para descargar video en 1080p usando yt-dlp y ffmpeg
rem Coloca este .bat junto a una carpeta "src" que contenga ffmpeg.exe y yt-dlp-*.exe

set "SCRIPT_DIR=%~dp0"
set "SRC_DIR=%SCRIPT_DIR%src\"

rem Buscar yt-dlp: primero en src, luego en la carpeta del script
...
if "%URL%"=="" (
    echo No se proporciono ninguna URL.
    pause
    exit /b 1
)

rem Nombre de salida opcional (sin extension)
if "%~2"=="" (
    set "OUT_NAME="
) else (
    set "OUT_NAME=%~2"
...
rem Seleccionar formato: mejor video hasta 1080p + mejor audio, y unir con ffmpeg en mp4
set "YT_FORMAT=bestvideo[height<=1080]+bestaudio/best[height<=1080]"

if "%OUT_NAME%"=="" (
    "%YTDLP%" --ffmpeg-location "%FFMPEG_LOC%" -f "%YT_FORMAT%" --merge-output-format mp4 -o "%SCRIPT_DIR%%%(title)s.%%(ext)s" "%URL%"
) else (
    "%YTDLP%" --ffmpeg-location "%FFMPEG_LOC%" -f "%YT_FORMAT%" --merge-output-format mp4 -o "%SCRIPT_DIR%%OUT_NAME%.mp4" "%URL%"
)
